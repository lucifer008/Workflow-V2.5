    ...
    protected void btnDisPrint_Click(object sender, EventArgs e)
    {
        try
        {
            ...
            if (null != ViewState["OrderNo"] && "" != ViewState["OrderNo"].ToString())
            {
                ...
                strQuery += "订单号:" + ViewState["OrderNo"].ToString();
            }
            if (null != searchAchievementAction.OrderModel.NewOrder)
            {
                ...
                achievement.NewOrderName = searchAchievementAction.OrderModel.NewOrder.ProjectName;//订单工程名
                achievement.TrashPaper = searchAchievementAction.OrderModel.NewOrder.SumAmount;//订单总计金额
            ...
            rart.CreateReport(searchAchievementAction.Model, "订单业绩分配统计", "订单业绩分配统计", PageSize.A4, 10, 10, 10, 10);
            Page.ClientScript.RegisterStartupScript(this.GetType(), "pdf", "<script type=''>showPage('../reports/" + reportFileName + "', '订单业绩分配统计', 1024, 1024 , false, false, null);</script>");
