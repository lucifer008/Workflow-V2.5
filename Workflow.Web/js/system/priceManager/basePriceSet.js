...
function showBatchDeletePrice(){
    var slt=$($("#sltBusinessType")[0]).attr("value");
    showPage('BatchDeletePrice.aspx' , "curWindow", 900, 700, false, false);//门市价格批量录入接口
}
$(document).ready(function(){
  $("input:button[@value=查询]").click(queryPriceSet);
  $("input:button[@value=单个录入]").click(showAddPrice);
  $("input:button[@value=批量录入]").click(showBatchAddPrice);
  $("input:button[@value=批量修改]").click(showBatchUpdatePrice);
});
formSubmit=function(){
    $("#MainForm").submit();
}
...
function showBatchUpdatePrice(){
	showPage('BatchUpdatePrice.aspx?id=1' , "curWindow", 1100, 600, false, false);
}


function arrangUpdatePrice(){
	if(isNaN($("#txtCostPrice").val()))
	{
		alert("请输入有效金额!!!");
		$("#txtCostPrice").focus();
		return false;
	}
	if(isNaN($("#txtStandardPrice").val()))
	{
		alert("请输入有效金额!!!");
		$("#txtStandardPrice").focus();
		return false;
	}
	if(isNaN($("#txtActivityPrice").val()))
	{
		alert("请输入有效金额!!!");
		$("#txtActivityPrice").focus();
		return false;
	}
	if(isNaN($("#txtReservePrice").val()))
	{
		alert("请输入有效金额!!!");
...
function selectedAllClick(){
    ...
    var arrDeleId=$("input:checkbox[@name=cbBBTPS]");
     if(cbStatus){   
