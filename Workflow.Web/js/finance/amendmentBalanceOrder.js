...
var trHtmlText = "";
var trDetailHtml="";
var processFlag=false; //回调函数是否执行完毕的标志
var orderDetailIsHidden=true;//指示当前的开单详细信息是显示还是隐藏
...
function setPageDefaultProperty(){
     //隐藏/显示开单时的复杂信息
...
function getSelectArray(index){
   ...
   if(sltArr.length<=1) setTimeout("getSelectArray("+index+")",500);
   else transficationOrderItemFactorValue(strNo,oSelect);
}
    ...
    for(var m=sltNum;m<sltArr.length;m++){
        ...
        for(var i=factorNum;i<data.OrderItemList.length;i++){
            if(data.OrderItemList[i].Id==data.OrderList[lastIndex-1].Id){
                isEdmit=data.OrderList[lastIndex-1].Memo!=undefined?false:true;
...
function showPrintRequest(aPrintRequest){
	aObj=aPrintRequest;
	showPage('../../order/PrintRequest.aspx', null, 800, 485, false, false);
}
...
function CustomerInfoCallBack(json){
    ...
    //提取原客户下明细转变为会员开户时明细的会员价格
    var ttMe=new toTakeMemberCardPrice("");
    ttMe.setOrderItemPriceToMemPrice();
...
//功能: 普通客户转向会员客户更改订单明细价格
//作者: 张晓林
//日期: 2010年7月19日17:38:55
var Class={
    create:function(){
        return function(){this.initialize.apply(this,arguments);};
    }
}
var toTakeMemberCardPrice=Class.create();
toTakeMemberCardPrice.prototype={
    initialize:function(o){
        this.orderNo=o;    
    },
    setOrderItemPriceToMemPrice:function(){
        var orderItemArr=$("input:hidden[@name=orderItemId]");
        for(var i=0;i<orderItemArr.length;i++){
            var edmitButton=$("input:button[@value=编辑]",$(orderItemArr[i]).parent().parent());
            var confirmButton=$("input:button[@value=确定]",$(orderItemArr[i]).parent().parent());
            $(edmitButton).attr("disabled",false);
            $(confirmButton).attr("disabled",false);
            $(edmitButton).click();
            hideControl(confirmButton);
            $(edmitButton).attr("disabled",true);
