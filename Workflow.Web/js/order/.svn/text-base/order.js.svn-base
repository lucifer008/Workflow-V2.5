...
function processJson(json, sourceSelect){
    ...
    //处理开单默认值
    processDefaultData(sourceSelect);
}
...
function addRow() {
    ...
    $("td:first-child", $("tr:last-child", table)).html(lastIndex);
     ...
     //add by baixiaoyu 2010-05-10 添加焦点设置
     if (sltArr && sltArr[0]){
		sltArr[0].focus();	
     }
...
function hideControl(button,submit){
    ...
    var txtArr=$("input[@type='text']",$(button).parent().parent());
    for(var i=0;i<txtArr.length;i++){
		if (txtArr[i].name == "Amount") continue;
        var strTmp='<span>'+txtArr[i].value+'</span>';
        $(strTmp).appendTo(txtArr[i].parentNode);
        $(txtArr[i]).hide();
    }
...
function orderDataCheck(o){
    ...
    var operateType=$(o).attr("id");
    var rowCount = $("#tbOrderItem")[0].rows.length;
    
    if("btnPrintOrder"==operateType){
        var sltArr=$("select",$());
        for(var i=0;i<sltArr.length;i++){
            if(-1==sltArr[i].selectedIndex) continue;
            if(sltArr[i].options[sltArr[i].selectedIndex].value==-1){
            
				var rowIndex = $(sltArr[i]).parent().parent()[0].rowIndex;
				if ((rowIndex+1) == rowCount && rowIndex != 1) continue;
                if(sltArr[i].name=="BusinessType"){
                    alert("请您选择业务类型!!!");
                    sltArr[i].focus();
                    return false;
        ...
        for(var i=0;i<amntArr.length;i++){
            ...
            var rowIndex = $(amntArr[i]).parent().parent()[0].rowIndex;
            if ((rowIndex+1) == rowCount && rowIndex != 1) continue;
            if(amountInput==""){
                alert("请输入数量!!!");
    ...
    var operateButtonName=$(o).attr("id");
    if("btnPrintOrder"==operateButtonName) $("#TagReq").val(1);
    if("btnDispatchOrder"==operateButtonName) $("#TagReq").val(2);
    if("btnWaitDispatchOrder"==operateButtonName) $("#TagReq").val(3);
...
//    var e=window.event||arguments[0];
//    if(116==e.keyCode){
//         window.location.href="DailyOrder.aspx";
//    }
//    else if(e.ctrlKey && e.shiftKey && e.keyCode==11){
//           window.location.href="../LoginOut.aspx";
//    }
//    alert(e.keyCode); 
//}
...
//打开前台直接分配订单页面
//陈汝胤 2010.03.24
function openDispatchOrderPage(){
	try{
		if(dispatchOrderNo){
			showPage('DispatchOrderForOrderItem.aspx?strNo='+escape(dispatchOrderNo)+'&strName='+escape(dispatchCustomerName)+'&IsPrint='+escape(isPrint), null, 780, 800, false, false);
...
//功能: 开单直接打印
//作者: 张晓林
//日期: 2010年3月29日9:39:09
function refresh(){
  try{
        if(rValOrder){
            document.frames['mframe'].document.getElementById('orderNo').value=orderNo;
            //alert(document.frames['mfram'].document.getElementById('orderNo').value);
            //document.frames('mframe').location.reload();//不能传参数
            //document.frames['mfram'].document.form1.submit();
            var queryStr="";
            if(null!=orderNo) queryStr+="?orderNo="+orderNo
            document.frames['mfram'].window.location.href="TodayOrder.aspx"+queryStr;
            //document.frames['mfram'].window.location.href=document.frames['mfram'].window.location.href;
            //document.frames['mfram'].window.location.href="TodayOrder.aspx"+queryStr;
            //document.frames['mfram'].navigate("TodayOrder.aspx"+queryStr);
            rValOrder=false;
        }
    }
    catch(e){
...
//功能: 处理开单默认值
//作者: 白晓宇
//日期: 2010年7月3日14:31:11
function processDefaultData(o){
    //debugger;
    var provider = {};
    provider.par=o;
    provider.process = function(json) {
        setDefaultData(json,provider.par);
    };
    $.getJSON("../ajax/AjaxEngine.aspx", {a : '39'},provider.process);
}

//功能: ajax返回
//作者: 白晓宇
//日期: 2010年7月3日15:21:16
function setDefaultData(defaultData,o){
    if(defaultData){
        //获取业务类型
        var defaultItem = null;
        for(var i = 0; i < defaultData.length; i++){
            if (o.value == defaultData[i].btid){
        ...
        var trArr = $('tr', $(trObj.parent()));
        var thArr = $('th', $(trArr[0]));
        //获取当前行中所有的td标签
        var tdArr = $('td',$(trObj));
        for(var i = 0; i < tdArr.length; i++){
            var sltObject = $('select',$(tdArr[i]));
            if ( sltObject.length){
                var temp = i -1;
                
                var  priceFactorId = null;
                if (thArr[i].id && thArr[i].index){
                    priceFactorId = thArr[i].id.split('th')[1];
                }
                
                for(var j = 0; j < defaultItem.pfa.length; j++){
                    if (defaultItem.pfa[j].id == priceFactorId){
                         for(var m = 0; m < sltObject[0].options.length; m++){
                            if (defaultItem.pfa[j].v == sltObject[0].options[m].value){
                                sltObject[0].options[m].selected = true;
                            }
                        }
                    }
                }
        ...
        //数量输入框 获取焦点
       var inputAmountArr = $("input:text[@name=Amount]", $(trObj)); 
       if (inputAmountArr && inputAmountArr.length == 1){
            inputAmountArr[0].focus();
       }
    }
}