...
var outObject;
var outButton;		//点击的按钮
var minDate;        //最小时间    //Author:Cry Date:2008-12-28 判断当前时间不能比目前时间小
var outDate="";		//存放对象的日期
...
var bUseDay=true;      //是否显示日
var bUseMonth=true;     //是否显示月
...
function setTime(tt,obj,hasTime,canUseTime,date)
{
...
function setday(tt,obj,useMonth,useDay) //主调函数
{
	if (arguments.length >  4){alert("对不起！传入本控件的参数太多！");return;}
	if (arguments.length == 0){alert("对不起！您没有传回本控件任何参数！");return;}
	if (arguments.length==4)  {bUseDay=useDay;bUseMonth=useMonth;bCanUseTime=false;}//如果传递的参数为4个时，只显示:年和月份----张晓林(修正时间:2009年4月24日13:53:22)
	var dads  = document.all.endDateLayer.style;
	var th = tt;
	var ttop  = tt.offsetTop;     //TT控件的定位点高
    ...
    minDate=null; //Author:Cry Date:2008-12-27
	//根据当前输入框的日期显示日历的年月
...
function meizzToday()  //Today Button
  {
    ...
    if(outObject){
		...
		else
		{
		    if(!bUseMonth)
		    {
			    outObject.value= parent.meizzTheYear ; //注：在这里你可以输出改成你想要的格式
		        
		    }
		    else if(!bUseDay)
		    {
			    outObject.value= parent.meizzTheYear + "-" + format( parent.meizzTheMonth); //注：在这里你可以输出改成你想要的格式
...
{
    //Modify:Cry Date:2008-12-27 修改可时间的判断
    var dateValue="";
	if (bUseTime)
	{
	    dateValue= yy + "-" + format(mm) + "-" + format(n) + " " + format(hh) + ":" + format(mi) + ":" + "00"; //注：在这里你可以输出改成你想要的格式
	}
	else
	{
	    ...
	    if(!bUseMonth)
	    {
		    dateValue= yy ; //注：在这里你可以输出改成你想要的格式
	        
	    }
	    else if(!bUseDay)
	    {
		    dateValue= yy + "-" + format(mm); //注：在这里你可以输出改成你想要的格式
	        
	    }
	    else
	    {
		    dateValue= yy + "-" + format(mm) + "-" + format(n); //注：在这里你可以输出改成你想要的格式
		}
    ...
    if(minDate){
        if(!CompareDate(dateValue,minDate)){
            alert('选择的时间不能比当前时间早');
            return;
        }
    }
    outObject.value=dateValue;
}
