...
public partial class SearchCustomer : Workflow.Web.PageBase
{
	...
	protected void Page_Load(object sender, EventArgs e)
    {
        ...
        int operateType = int.Parse(hidDate.Value);
        if (999 != operateType) QueryDataByDate(operateType);
    }
    ...
    private void QueryDataByDate(int day)
    {
        ...
        ViewState.Add("SecondaryTradeId", null);
        ViewState.Add("CustomerLevelId", null);
        ViewState.Add("CustomerTypeId", null);
    ...
    protected void SearchCustomerInfo(object sender, EventArgs e)
    {
        try
        {
            ...
            if (!StringUtils.IsEmpty(MemberCardNo.Text))
            {
                model.MemberCardNo = MemberCardNo.Text;
            }
            else 
            {
                model.MemberCardNo = null;
            }
            ...
            this.ViewState.Add("LinkMan", model.LinkMan);
            this.ViewState.Add("MemberCardNo", model.MemberCardNo);
    ...
    private void CurrentNextPageData(int currentPageIndex) 
    {
        ...
        if (null != ViewState["MemberCardNo"]) 
        {
