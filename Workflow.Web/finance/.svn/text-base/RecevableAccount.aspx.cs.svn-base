...
/// 创建时间: 2007-10-13
/// 修正履历: 张晓林
/// 修正时间: 增加预存款冲减,添加预存款,开发票功能,税费收取功能
/// </remarks>
public partial class RecevableAccount : Workflow.Web.PageBase
{
    #region //Class Member
    protected decimal scotInverse = 0;
    private bool isPaidAmount = false;//是否开票
    private decimal paidTicketAmount = 0;//开票金额
    ...
    {
        scotInverse = Convert.ToDecimal(financeAction.GetScotInverse());
        if (!IsPostBack){}
    ...
    private void Save() 
    {
        try
        {
            string[] orderIdArr = Request.Form["OrdersId"].Split(',');//付款订单列表
            string[] money = Request.Form["Money"].Split(',');        //付款订单付款金额列表
            string[] accMoney = Request.Form["txtAccMoney"].Split(',');//付款订单应收金额列表
            financeAction.Model.OrderList = new List<Order>();
            financeAction.Model.GatheringList = new List<Gathering>();
            financeAction.Model.DiffArrOrderList = new List<string>();//记录不欠款的订单Id
            //预存款冲减
            ...
            for (int i = 0; i < orderIdArr.Length; i++)
            {
                ...
                if (amountDiff > 0)//使用预存款
                {
                    if ((amountDiff + concessionAmount) >= decimal.Parse(money[i]))//仅用预存款冲减
                    {
                        ...
                        or.RealPaidAmount = or.PaidAmount;
                        ...
                        if (Convert.ToDecimal(accMoney[i]) == Convert.ToDecimal(money[i]))
                            financeAction.Model.DiffArrOrderList.Add(orderIdArr[i]);
                    }
                    else //预存款冲减和现金冲减
                    {
                        Order or = new Order();
                ...
                else
                {
                    ...
                    order.RealPaidAmount = order.PaidAmount;
            ...
            //开发票，统计税费
            if (null != Request.Form["txtPaidTicketAmount"] && "0" != Request.Form["txtPaidTicketAmount"])
            {
                isPaidAmount = true;
                paidTicketAmount = Convert.ToDecimal(Request.Form["txtPaidTicketAmount"]);
                financeAction.Model.IsPaidTicket = true;
                OtherGatheringAndRefundmentRecord or = new OtherGatheringAndRefundmentRecord();
                or.CustomerId = Convert.ToInt32(Request.Form["CustomerId"]);
                or.Amount = Convert.ToDecimal(Request.Form["txtScotAmount"]);
                or.TicketAmountSum = Convert.ToDecimal(Request.Form["txtPaidTicketAmount"]);
                or.PayKind = Constants.ACCOUNT_SCOT_AMOUNT_KEY.ToString();
                or.OrdersId = 0;
                or.Memo = "应收款处理:税费及开票金额";
                financeAction.Model.OtherGatheringAndRefundmentRecord = or;
            }
    ...
    private void Print()
    {
        ...
        financeAction.Model.IsAccounting = isPaidAmount;//是否开票
        financeAction.Model.PaidAmount = paidTicketAmount;
