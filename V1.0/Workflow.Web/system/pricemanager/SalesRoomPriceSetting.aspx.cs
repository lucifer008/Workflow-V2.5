        ...
        if (null != model.PriceFactor && model.PriceFactor.Count > 0)
        {
            foreach(PriceFactor priceFactor in model.PriceFactor)
            {
                if(currentmap!=null && this.isQuery.Value == "0")
                {
                    ...
                    {
                        model.BaseBusinessTypePriceSet[key] = (long)currentmap[key];
                    }
                }
                else
                {
                    string factorValue = Request.Form["factorValuex" + priceFactor.Id];
                    if (null != factorValue && factorValue!="-1")
                    {
                        try
                        {
                            model.BaseBusinessTypePriceSet[int.Parse(priceFactor.Id.ToString()) - 1] = long.Parse(factorValue);
                            currentmap.Add(int.Parse(priceFactor.Id.ToString()) - 1, long.Parse(factorValue));
                        }
                        catch (Exception ex)
                        {
                            Workflow.Support.Log.LogHelper.WriteError(ex, Constants.LOGGER_NAME);
