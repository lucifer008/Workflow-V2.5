        ...
        if (null != Request.QueryString["UserId"] && "" != Request.QueryString["UserId"].ToString())
        {
            ...
            if (userAction.Model.UserList.Count > 0)
            {
                ...
                foreach(UserConcessionRange ucr in userAction.Model.UserConcessionRanageList)
                {
                    ...
                    {
                        maxConcessionMoney = ucr.Max.ToString();
                        minConcessionMoney = ucr.Min.ToString();
                        continue;
                    }
                    //折让
                    if(ucr.ConcessionType==Constants.CONCESSION_TYPE_RENDERUP_VALUE)
                    {
                        maxDiscount = ucr.Max.ToString();
                        minDiscount = ucr.Min.ToString();
                        continue;
                    }
                }
            }
		...
		try
		{
			...
			if (Request.Form["cbBoxRole"] != null)
			{
                ...
                if("True"==hiddTag.Value)
                {
                    ...
                    userConcessionCon.Max = Convert.ToDecimal(Request.Form["txtMaxConcessionMoney"]);
                    userConcessionCon.Min = Convert.ToDecimal(Request.Form["txtMinConcessionMoney"]);
                    ...
                    userConcessionDiscount.Max = Convert.ToDecimal(Request.Form["txtMaxDiscountMoney"]);
                    userConcessionDiscount.Min = Convert.ToDecimal(Request.Form["txtMinDiscountMoney"]);
