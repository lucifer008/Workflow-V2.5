...
function isDate(str)
{
    ...
    var reg="[0-9]{4}\-(0?[1-9]|10|11|12)\-(0?[1-9]|[1-2][0-9]|30|31)";
    var r = str.match(reg);
    //var r = str.match("/^([1-2]\d{3})[\/|\-](0?[1-9]|10|11|12)[\/|\-]([1-2]?[0-9]|0[1-9]|30|31)$/");
    
    if(r==null){alert('您输入的日期有误！！');return false;}
    var d= new Date(r[1], r[3]-1, r[4]);
    //return (d.getFullYear()==r[1] && (d.getMonth()+1)==r[3] && d.getDate()==r[4]);
    
    if((d.getFullYear()==r[1] || (d.getMonth()+1)==r[3] || d.getDate()==r[4]))
    {
        //alert("日期格式不正确！！"); //msh 20070824 deleted
        return false;
    }
}
...
function isDateTime(str)
{
    var reg =" /^(d)(-|/)(d)2(d) (d):(d)$/";
    var r = str.match(reg);
    if(r==null){alert('您输入的日期时间格式有误!!');return false;}
...
function lostFocus(o)
{
    ...
    var dateStr=objStr.substring(0,10);
    var timeStr=objStr.substring(11);
    if(!isDate(dateStr))
    {
//        alert('您输入的日期有误！！'); //msh 20070824 deleted
        o.value="";
        o.setCapture(true);
    }
    if(!isTime(timeStr))
