    ...
    public class ReportDailyPaper: ReportBase<iTextSharp.text.pdf.PdfPTable, OrderModel>
    {
       /// <summary>
		...
		private PdfPTable GetOrderItem(OrderModel u)
		{
			...
			///定义列标题
			///
			float [] ColumnWidth={ 10,15,15,15,15,20,20};
			...
			for (int j = 0; j < u.OrderList.Count; j++)
			{
              ...
              //PdfPCell pAccountReceviableOweMomeyTotal = new PdfPCell(new Paragraph(NumericUtils.ToMoney(u.OrderList[j].AccountReceviableOweMomeyTotal).ToString(), FontBase));//记账
              //处理当抹零金额没有添加到数据库的情况
