...
namespace Workflow.Support.Report
{
    ...
    public class ReportOrder : ReportBase<iTextSharp.text.pdf.PdfPTable, OrderModel>
    {
        ...
        private PdfPTable GetOrderCustomerInfo(OrderModel u)
        {
            ...
            //PdfPCell pc53 = new PdfPCell(new Paragraph("", FontBase));//new Paragraph("发票:", FontBase));
            //pc53.Border = 0;
            //pc53.NoWrap = false;
            //pc53.HorizontalAlignment = 0;
            //pc53.FixedHeight = ROW_HEIGHT;
            //ptb.AddCell(pc53);
            ...
            if ("Y" == u.NewOrder.NeedTicket)
            {
                ...
                //    //strTemp = u.NewOrder.RealPaidAmount.ToString();
                //    if (u.NewOrder.RealPaidAmount <= 0) //开单时 还没实收金额
                //    {
                //        strTemp = (u.NewOrder.SumAmount - concession).ToString("C");
                //    }
                //    //有预收 有舍入 预收金额使用后 没有剩余
                //    else if (Math.Abs(u.NewOrder.RealPaidAmount + u.NewOrder.PrepareMoneyAmount - u.NewOrder.SumAmount) < 1)
                //    {
                //        strTemp = (u.NewOrder.RealPaidAmount + u.NewOrder.PrepareMoneyAmount).ToString("C");
                //    }
                //    else
                //    {
                //        strTemp = (u.NewOrder.SumAmount - concession).ToString("C");
                //    }
                //}
                //else if ("Y" == u.NewOrder.PaidTicket)
                //{
                //    strTemp = u.NewOrder.PaidTicketAmount.ToString(); //"还欠 " + u.NewOrder.NotPayTicketAmount.ToString();
                //}
        ...
        private PdfPTable GetOrderItem(OrderModel u)
        {
            ...
            for (int x = 1; x <= u.PriceFactor.Count; x++)
            {
                aList.Add(50);
            }
            aList.Add(30);
            ...
            {
                PdfPCell pc1 = new PdfPCell(new Paragraph("", FontBase));//new Paragraph((i + 1).ToString(), FontBase));
                pc1.HorizontalAlignment = Element.ALIGN_BOTTOM;//Element.ALIGN_CENTER;
                pc1.Border = 0;
                pc1.FixedHeight = 22F;//ROW_HEIGHT;
                ptb.AddCell(pc1);

                PdfPCell pc2 = new PdfPCell(new Paragraph(u.OrderItem[i].Name, FontBase));
                pc2.HorizontalAlignment = Element.ALIGN_BOTTOM;//Element.ALIGN_CENTER;
                pc2.Border = 0;
                ...
                for (int n = position; n < u.NewOrder.OrderItemList.Count; n++)
                {
                    if (u.OrderItem[i].Id == u.NewOrder.OrderItemList[n].Id)
                    {
                        ...
                        for (int k = (int)(u.NewOrder.OrderItemList[n].PriceFactorId % Constants.MAX_ID_BASE); k < u.PriceFactor.Count; k++)
                        {
                            ...
                            {
                                PdfPCell pc = new PdfPCell(new Paragraph("-", FontBase));
                                pc.HorizontalAlignment = Element.ALIGN_BOTTOM;//Element.ALIGN_CENTER;
                                pc.Border = 0;
                            ...
                            {
                                PdfPCell pc = new PdfPCell(new Paragraph(u.NewOrder.OrderItemList[n].Name, FontBase));
                                pc.HorizontalAlignment = Element.ALIGN_BOTTOM;//Element.ALIGN_CENTER;
                                pc.Border = 0;
                                pc.FixedHeight = 22F;//ROW_HEIGHT;
                                ptb.AddCell(pc);
                ...
                PdfPCell pAmount = new PdfPCell(new Paragraph(u.OrderItem[i].Amount == 0 ? "0" : u.OrderItem[i].Amount.ToString(), FontBase));
                pAmount.HorizontalAlignment = Element.ALIGN_BOTTOM;//Element.ALIGN_RIGHT;
                pAmount.Border = 0;
                pAmount.FixedHeight = 22F;//ROW_HEIGHT;
                ptb.AddCell(pAmount);

                PdfPCell pUnitPrice = new PdfPCell(new Paragraph(u.OrderItem[i].UnitPrice == 0 ? "0.00" : u.OrderItem[i].UnitPrice.ToString("C"), FontBase));
                pUnitPrice.HorizontalAlignment = Element.ALIGN_BOTTOM;//Element.ALIGN_RIGHT;
                pUnitPrice.Border = 0;
                pUnitPrice.FixedHeight = 22F;//ROW_HEIGHT;
                ptb.AddCell(pUnitPrice);

                PdfPCell pSum = new PdfPCell(new Paragraph((u.OrderItem[i].Amount * u.OrderItem[i].UnitPrice) == 0 ? "0.00" : (u.OrderItem[i].Amount * u.OrderItem[i].UnitPrice).ToString("C"), FontBase));
                pSum.HorizontalAlignment = Element.ALIGN_BOTTOM;// Element.ALIGN_RIGHT;
                pSum.Border = 0;
                ...
                PdfPCell pPrintRequest = new PdfPCell(new Paragraph(strPrint, FontBase));
                pPrintRequest.HorizontalAlignment = Element.ALIGN_BOTTOM;//Element.ALIGN_RIGHT;
                pPrintRequest.Border = 0;
                pPrintRequest.FixedHeight = 22F;//ROW_HEIGHT;
                ptb.AddCell(pPrintRequest);

            }
            for (int i = u.OrderItem.Count; i < 15; i++)
            {
                PdfPCell pc1 = new PdfPCell(new Paragraph("", FontBase));//new Paragraph((i + 1).ToString(), FontBase));
            ...
            {
                //pCountUpper = new PdfPCell(new Paragraph(NumericUtils.ConversionMoney((sumMoney - concession).ToString()) +
                //    strCountUpper, FontBase));
                //pCount = new PdfPCell(new Paragraph((sumMoney - concession).ToString("C"), FontBase));
                decimal reAmount = sumMoney - concession;
                if (u.IsAccounting) 
                {
                    if (u.IsUsePreAmount) reAmount = sumMoney - concession;
                    //if (u.IsUsePreDeposits) reAmount -= u.PreDeposits;
                }
            ...
            else
            {
                ...
                if(u.IsAccounting)
                {
                    //if (u.IsUsePreAmount)   reAmount += u.NewOrder.PrepareMoneyAmount;
                    //if (u.IsUsePreDeposits) reAmount += u.PreDeposits;
                    if (u.IsUsePreAmount) reAmount = sumMoney - concession;
                }
                //else    
                ...
                //    //if (u.IsUsePreDeposits) reAmount -= u.PreDeposits;
                //}
                pCountUpper = new PdfPCell(new Paragraph(NumericUtils.ConversionMoney(reAmount.ToString()) +
                   strCountUpper, FontBase));
                pCount = new PdfPCell(new Paragraph(reAmount.ToString("C"), FontBase));
            }

            //有预收 有舍入 预收金额使用后 没有剩余
            //if (Math.Abs(u.NewOrder.RealPaidAmount + u.NewOrder.PrepareMoneyAmount+u.PreDeposits - u.NewOrder.SumAmount) < 1)
            //{
            //    pCountUpper = new PdfPCell(new Paragraph(NumericUtils.ConversionMoney((u.NewOrder.RealPaidAmount + u.NewOrder.PrepareMoneyAmount + u.PreDeposits).ToString()) +
            //    strCountUpper, FontBase));
            //    pCount = new PdfPCell(new Paragraph((u.NewOrder.RealPaidAmount + u.NewOrder.PrepareMoneyAmount + u.PreDeposits).ToString("C"), FontBase));
            //}
            //else if (Math.Abs(u.NewOrder.RealPaidAmount + u.NewOrder.PrepareMoneyAmount + u.PreDeposits - u.NewOrder.SumAmount) > 1)//有预收但未使用--张晓林--修正时间:2009年9月10日15:20:01
            //{
            //    pCountUpper = new PdfPCell(new Paragraph(NumericUtils.ConversionMoney((u.NewOrder.RealPaidAmount + u.PreDeposits).ToString()) +
            //    strCountUpper, FontBase));
            //    pCount = new PdfPCell(new Paragraph((u.NewOrder.RealPaidAmount + u.PreDeposits).ToString("C"), FontBase));
            //}
            //else
            //{
            //    pCountUpper = new PdfPCell(new Paragraph(NumericUtils.ConversionMoney((sumMoney - concession).ToString()) +
