...
namespace Workflow.Action
{
    public  class MemberCardAction
    {
        ...
        /// 修正时间:
        /// </remarks>
        public virtual void InsertMemberCardConcession()
        {
            memberCardService.InsertConcessionMemberCard(model.MemberCardConcession);//, model.ChargeSum);
        }
        #endregion
        ...
        /// 功能概要:会员卡冲减
        /// 作    者:张晓林
        /// 创建时间:2009年4月15日17:35:37
        /// 修正履历:
        /// 修正时间:
        /// </summary>
        public void MemberCardDiff() 
        {
            model.MemberCardConcessionList = searchMemberCardService.GetMemberCardConcessionBaseBusniessInfo(model.MemberCard.Id);
            model.OrderList = searchMemberCardService.GetMemberCardBaseBusItem(model.MemberCard);
            if(0!=model.OrderList.Count && 0!=model.MemberCardConcessionList.Count)
            {
                memberCardService.MemberCardDiff(model.MemberCardConcessionList, model.OrderList,model.MemberCard);
            }
        }
        #endregion

        internal void SearchMemberCardByCardNo(string memberCardNo)
        {
