...
namespace Workflow.Service.SystemManege.PriceManage
{
    class PriceServiceImpl : IPriceService
    {
        ...
        public void DeleteBusinessType(BusinessType businessType)
        {
            ...
            businessTypePriceFactorDao.DeleteByBusinessTypeId(businessType.Id);
            //编辑业务类型
            businessTypeDao.LogicDelete(businessType.Id);
        }
        #endregion
        ...
        {
            baseBusinessTypePriceSetDao.DeleteBusinessTypePriceSet(baseBusinessTypePriceSet.Id);
            //concessionDao.DeleteConcessionDiffrentPrice(baseBusinessTypePriceSet.Id);
            //concessionDao.DeleteConcessionMemberCardLevel(baseBusinessTypePriceSet.Id);
            concessionDao.DeleteConcession(baseBusinessTypePriceSet.Id);
        ...
        [Transaction]
        public void BatchDeletePrice(IList<string> strBaseBusinessTypePriceSetList) 
        {
            foreach(string str in strBaseBusinessTypePriceSetList)
            {
                baseBusinessTypePriceSetDao.DeleteBusinessTypePriceSet(Convert.ToInt32(str));
                concessionDao.DeleteConcession(Convert.ToInt32(str));
                baseBusinessTypePriceSetDao.Delete(Convert.ToInt32(str));
            }
        }
        ...
        {
            return businessTypePriceSetDao.GetBusinessTypePriceSetListCustomQueryCount(businessTypePriceSet);
        }
		...
		#region 设置过的会员或者行业价格,都按条件指定条件(不分页)
		...
		public IList<BusinessTypePriceSet> GetOnlyBusinessTypePriceSetList(BusinessTypePriceSet businessTypePriceSet)
		{
			...
			//门市价格设置List
			IList<BusinessTypePriceSet> businessTypePriceSetList;
			businessTypePriceSetList = businessTypePriceSetDao.GetOnlyBusinessTypePriceSetList(businessTypePriceSet);
        ...
        {
            businessTypePriceSetDao.LogicDelete(businessTypePriceSet.Id);
        }
        #endregion
		...
		public IList<BaseBusinessTypePriceSet> GetBaseBusinessTypePrices(BaseBusinessTypePriceSet baseBusinessTypePriceSet)
		{
			int inti = 0, intj = 0, intk = 0;
			int intFoundPos = 0;
			bool blnFound;
			IList<BaseBusinessTypePriceSet> baseBusinessTypePriceSetList = baseBusinessTypePriceSetDao.GetBaseBusinessTypePriceSets(baseBusinessTypePriceSet);
			//价格因素
			IList<PriceFactor> priceFactorList;
			priceFactorList = masterDataService.GetPriceFactors();
		...
		#region 活动订单号获取订单明细的门市价格

		/// <summary>
		/// 活动订单号获取订单明细的门市价格
		/// </summary>
		/// <param name="orderNo">订单号</param>
		/// <returns>订单明细的门市价格</returns>
		/// <remarks>
		/// 作    者: 陈汝胤
		/// 创建时间: 2009.5.19
		/// </remarks>
		public IList<OrderItem> GetOrderItemBaseBusinessTypePriceSet(string orderNo)
		{
			IList<OrderItem> orderItemList = orderService.GetOrderItemByOrderNo(orderNo);
			IList<OrderItem> orderItemFactorValueList = orderService.GetCashOrderItemFactorValueByOrderId(orderNo);
			Hashtable map = new Hashtable();
			BaseBusinessTypePriceSet baseBusinessTypePriceSet = null;
			foreach (OrderItem orderItem in orderItemFactorValueList)
			{
				if (map.ContainsKey(orderItem.Id))
				{
					baseBusinessTypePriceSet = (BaseBusinessTypePriceSet)map[orderItem.Id];
					baseBusinessTypePriceSet[Convert.ToInt32(orderItem.PriceFactorId) - 1] = long.Parse(orderItem.Values);
					if (orderItem.TargetTable == "MACHINE_TYPE")
					{
						baseBusinessTypePriceSet.MachinePriceFactor = long.Parse(orderItem.Values);
					}
					if (orderItem.TargetTable == "PAPER_SPECIFICATION")
					{
						baseBusinessTypePriceSet.PaperPriceFactor = long.Parse(orderItem.Values);
					}
				}
				else
				{
					baseBusinessTypePriceSet = new BaseBusinessTypePriceSet();
					baseBusinessTypePriceSet.BusinessTypeId = orderItem.BusinessTypeId;
					baseBusinessTypePriceSet[Convert.ToInt32(orderItem.PriceFactorId) - 1] = long.Parse(orderItem.Values);
					if (orderItem.TargetTable == "MACHINE_TYPE")
					{
						baseBusinessTypePriceSet.MachinePriceFactor = long.Parse(orderItem.Values);
					...
					map.Add(orderItem.Id, baseBusinessTypePriceSet);
				}
			}
			foreach (OrderItem orderItem in orderItemList)
			{
				if (map.ContainsKey(orderItem.Id))
				{
					BaseBusinessTypePriceSet tempBaseBusinessTypePriceSet = (BaseBusinessTypePriceSet)map[orderItem.Id];
					baseBusinessTypePriceSet = priceService.GetPrice(tempBaseBusinessTypePriceSet);
					orderItem.BaseBusinessTypePriceSetId = baseBusinessTypePriceSet.Id;
					orderItem.MachinePriceFactor = tempBaseBusinessTypePriceSet.MachinePriceFactor;
					orderItem.PaperPriceFactor = tempBaseBusinessTypePriceSet.PaperPriceFactor;
				}
			}
        ...
        /// </summary>
        [Transaction]
        public void DeleteMembercardPrice(IList<BusinessTypePriceSet> businessTypePriceSetList) 
        {
            foreach (BusinessTypePriceSet btps in businessTypePriceSetList)
            {
                businessTypePriceSetDao.DeleteMembercardPrice(btps);
            }
        }
        #endregion
        ...
        {
            businessTypePriceSetDao.UpdateMembercardPrice(businessTypePriceSet);
        }
        #endregion
		...
		[Transaction]
		public void UpdateBaseBusinessTypePrice(IList<BaseBusinessTypePriceSet> iList)
		{
			foreach (BaseBusinessTypePriceSet val in iList)
			{
				baseBusinessTypePriceSetDao.UpdateBusinessTypePrice(val);
			}
		}
		...
		/// <param name="iList"></param>
		[Transaction]
		public void UpdateBusinessTypePrice(IList<BusinessTypePriceSet> iList)
		{
			foreach (BusinessTypePriceSet val in iList)
			{
				businessTypePriceSetDao.UpdateBusinessTypePrice(val);
			}
		}
		...
		/// 描    述:
		/// </summary>
		public BaseBusinessTypePriceSet GetTheMostBaseBusinessTypePrice(BaseBusinessTypePriceSet domain)
		{
			return baseBusinessTypePriceSetDao.SelectTheMostBaseBusinessTypePrice(domain);
		}
		#endregion
