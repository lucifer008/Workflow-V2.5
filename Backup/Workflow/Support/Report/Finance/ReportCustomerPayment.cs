...
namespace Workflow.Support.Report.Finance
{
	public class ReportCustomerPayment : ReportBase<iTextSharp.text.pdf.PdfPTable, FindFinanceModel>
	{
		...
		private PdfPTable GetOrderItem(FindFinanceModel u)
		{
			...
			cell = new PdfPCell(new Paragraph("付款日期", FontColumn));
			cell.NoWrap = true;
			cell.HorizontalAlignment = Element.ALIGN_CENTER;
			ptb.AddCell(cell);

			cell = new PdfPCell(new Paragraph("收款人", FontColumn));
			cell.NoWrap = true;
			cell.HorizontalAlignment = Element.ALIGN_CENTER;
			ptb.AddCell(cell);

			cell = new PdfPCell(new Paragraph("付款金额", FontColumn));
			cell.NoWrap = false;
			cell.HorizontalAlignment = Element.ALIGN_CENTER;
			ptb.AddCell(cell);

			cell = new PdfPCell(new Paragraph("付款类型", FontColumn));
			cell.NoWrap = false;
			cell.HorizontalAlignment = Element.ALIGN_CENTER;
			ptb.AddCell(cell);
