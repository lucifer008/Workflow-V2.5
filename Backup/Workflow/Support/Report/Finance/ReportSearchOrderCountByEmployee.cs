...
{
    /// <summary>
    /// 名    称: ReportSearchOrderCountByEmployee
    /// 功能概要: 按人员统计开单量输出报表
    /// 作    者: 张晓林
    /// 创建时间: 2009年11月16日13:24:29
    /// 修正履历: 
    /// 修正时间:
    /// </summary>
    public class ReportSearchOrderCountByEmployee: ReportBase<iTextSharp.text.pdf.PdfPTable, FindFinanceModel>
    {
        ...
        private PdfPTable GetOrderItem(FindFinanceModel u)
        {
            ...
            PdfPCell pH3 = new PdfPCell(new Paragraph("雇员", FontColumn));
            pH3.NoWrap = true;
            pH3.HorizontalAlignment = Element.ALIGN_CENTER;
            ptb.AddCell(pH3);

            PdfPCell pH4 = new PdfPCell(new Paragraph("岗位", FontColumn));
            pH4.NoWrap = true;
            pH4.HorizontalAlignment = Element.ALIGN_CENTER;
            ptb.AddCell(pH4);

            PdfPCell pH5 = new PdfPCell(new Paragraph("开单量", FontColumn));
            pH5.NoWrap = true;
