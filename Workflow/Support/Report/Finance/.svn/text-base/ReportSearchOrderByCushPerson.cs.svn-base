...
namespace Workflow.Support.Report.Finance
{
    ...
    public class ReportSearchOrderByCushPerson : ReportBase<iTextSharp.text.pdf.PdfPTable, OrderModel>
    {
		...
		private PdfPTable GetOrderItem(OrderModel u)
		{
			...
			///定义列标题
			///
            float[] ColumnWidth ={ 10, 20, 20, 20, 20, 20, 20, 20, 20 };
			...
			for (int j = 0; j < u.OrderList.Count; j++)
			{
                ...
                PdfPCell pcMemberDiffAmount = new PdfPCell(new Paragraph(NumericUtils.ToMoney(u.OrderList[j].RealPaidAmount), FontColumnValue));
                pcMemberDiffAmount.HorizontalAlignment = Element.ALIGN_RIGHT;
                ptb.AddCell(pcMemberDiffAmount);
           ...
           PdfPCell pcMemberDiffAmountTotal = new PdfPCell(new Paragraph(NumericUtils.ToMoney(membercardDiffAmountTotal), FontColumnValue));
           pcMemberDiffAmountTotal.HorizontalAlignment = Element.ALIGN_RIGHT;
           ptb.AddCell(pcMemberDiffAmountTotal);
