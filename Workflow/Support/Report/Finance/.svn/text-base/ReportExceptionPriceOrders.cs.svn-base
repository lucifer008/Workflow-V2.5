...
{
	public class ReportExceptionPriceOrders : ReportBase<iTextSharp.text.pdf.PdfPTable, FindFinanceModel>
	{
		/// <summary>
		/// 查询字符串条件
		/// </summary>
		...
		private PdfPTable GetOrderItem(FindFinanceModel u)
		{
			...
			///定义列标题
			///
			float[] ColumnWidth ={ 6,11, 16,9 ,9, 9, 9, 9, 9, 9, 9 };
			...
			cell = new PdfPCell(new Paragraph("收银人", FontColumn));
			cell.NoWrap = false;
			cell.HorizontalAlignment = Element.ALIGN_CENTER;
			ptb.AddCell(cell);
			...
			foreach (Order val in u.PrintOrderList)
			{
				...
				cell = new PdfPCell(new Paragraph(val.Name, FontColumnValue));
				cell.HorizontalAlignment = Element.ALIGN_CENTER;
				ptb.AddCell(cell);
