...
{
	public class ReportOrderSum : ReportBase<iTextSharp.text.pdf.PdfPTable, FindFinanceModel>
	{
		...
		private PdfPTable GetOrderItem(FindFinanceModel u)
		{
			...
			///定义列标题
			///
			float[] ColumnWidth ={ 7, 50, 15, 15, 15,30 };
			...
			cell = new PdfPCell(new Paragraph("制作数", FontColumn));
			cell.NoWrap = false;
			cell.HorizontalAlignment = Element.ALIGN_CENTER;
			ptb.AddCell(cell);
			...
			cell = new PdfPCell(new Paragraph(orderCount.ToString(), FontColumnValue));
			cell.NoWrap = true;
			cell.HorizontalAlignment = Element.ALIGN_CENTER;
			ptb.AddCell(cell);

			cell = new PdfPCell(new Paragraph(paperCount.ToString(), FontColumnValue));
			cell.NoWrap = true;
			cell.HorizontalAlignment = Element.ALIGN_CENTER;
			ptb.AddCell(cell);
