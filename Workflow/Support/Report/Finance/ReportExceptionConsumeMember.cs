...
namespace Workflow.Support.Report.Finance
{
    ...
    /// 功能概要: 异常消费会员查询报表
    /// 作    者: 张晓林 
    /// 创建时间: 2010年2月3日13:58:47
    /// 修正履历: 
    /// 修正时间: 
    /// 修正描述: 
    /// </summary>
    public class ReportExceptionConsumeMember : ReportBase<iTextSharp.text.pdf.PdfPTable, FindFinanceModel>
    {
        /// <summary>
        /// 查询字符串条件
        /// </summary>
        ...
        private PdfPTable GetOrderItem(FindFinanceModel u)
        {
            ...
            PdfPCell pH3 = new PdfPCell(new Paragraph("会员卡号", FontColumn));
            ...
            PdfPCell pHProjectName = new PdfPCell(new Paragraph("消费金额", FontColumn));
            pHProjectName.NoWrap = true;
            pHProjectName.HorizontalAlignment = Element.ALIGN_CENTER;
            ptb.AddCell(pHProjectName);

            PdfPCell pH5 = new PdfPCell(new Paragraph("同比", FontColumn));
            ...
            for (int j = 0; j < u.OrderTempList.Count; j++)
            {
                ...
                PdfPCell pcOrderNo = new PdfPCell(new Paragraph(u.OrderTempList[j].MemberCardNo, FontColumnValue));
                pcOrderNo.HorizontalAlignment = Element.ALIGN_CENTER;
                ptb.AddCell(pcOrderNo);

                PdfPCell pcCustomerValue = new PdfPCell(new Paragraph(u.OrderTempList[j].CustomerName, FontColumnValue));
                pcCustomerValue.HorizontalAlignment = Element.ALIGN_RIGHT;
                ptb.AddCell(pcCustomerValue);

                PdfPCell pcProjectName = new PdfPCell(new Paragraph(Workflow.Util.NumericUtils.ToMoney(u.OrderTempList[j].SumAmount), FontColumnValue));
                pcProjectName.HorizontalAlignment = Element.ALIGN_RIGHT;
                ptb.AddCell(pcProjectName);

                PdfPCell pcInsertPerson = new PdfPCell(new Paragraph(Math.Round(u.OrderTempList[j].SumAmount / u.OrderTempList[j].HasPrePaidMoney, 2) * 100 + "%", FontColumnValue));
