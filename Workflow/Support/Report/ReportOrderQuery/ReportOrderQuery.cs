...
namespace Workflow.Support.Report.ReportOrderQuery
{
  ...
  public   class ReportOrderQuery : ReportBase<iTextSharp.text.pdf.PdfPTable, OrderModel>
  {
      ...
      {
          float[] widths ={ 5, 15, 20, 20, 15, 15, 12, 12, 10 };
      ...
      public override PdfPTable GetReportContent(OrderModel u)
      {
         ...
         // PdfPTable ptb = new PdfPTable(widths);
         // PdfPCell pc1 = new PdfPCell(new Paragraph("订单号:", FontColumn));
         // pc1.NoWrap = true;
      ...
      private PdfPTable GetOrderItem(OrderModel u)
      {
          ...
          ptb.AddCell(pH1);
          PdfPCell pH2 = new PdfPCell(new Paragraph("订单号", FontColumn));
          ...
          PdfPCell pH3 = new PdfPCell(new Paragraph("客户名称", FontColumn));
              ...
              for (int i = 0; i < u.OrderTempList.Count; i++)
              {
                  ...
                  ptb.AddCell(pc5);
                  PdfPCell pc6 = new PdfPCell(new Paragraph(NumericUtils.ToMoney(u.OrderTempList[i].Sumamount), FontColumnValue));
                  pc6.HorizontalAlignment = Element.ALIGN_RIGHT;
                  ptb.AddCell(pc6);
                  ...
                  ptb.AddCell(pc9);
                  orderSumAmountTotal += u.OrderTempList[i].Sumamount;
              }
              PdfPCell pcTotalColumn = new PdfPCell(new Paragraph("合计",FontColumnValue));
              pcTotalColumn.Colspan = 1;
              ptb.AddCell(pcTotalColumn);

              PdfPCell pcTotalColumnValue = new PdfPCell(new Paragraph(NumericUtils.ToMoney(orderSumAmountTotal), FontColumnValue));
              pcTotalColumnValue.Colspan = 8;
              pcTotalColumnValue.HorizontalAlignment = Element.ALIGN_RIGHT;
              ptb.AddCell(pcTotalColumnValue);
          return ptb;
      }
