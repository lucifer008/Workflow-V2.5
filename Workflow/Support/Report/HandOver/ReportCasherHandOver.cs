...
namespace Workflow.Support.Report.HandOver
{
    ...
    public class ReportCasherHandOver : ReportBase<iTextSharp.text.pdf.PdfPTable, CashHandOverModel>
    {
        ...
        private PdfPTable GetOrderItem(CashHandOverModel u)
        {
            ...
            PdfPCell pcOrderNo = new PdfPCell(new Paragraph("订单号", FontColumn));
            ...
            //会员卡充值金额合计
            PdfPCell pcMemTotalCo = new PdfPCell(new Paragraph(Convert.ToString(index + 2), FontColumnValue));
            pcMemTotalCo.HorizontalAlignment = Element.ALIGN_CENTER;
            ptb.AddCell(pcMemTotalCo);

            PdfPCell pcMemTotalCo2 = new PdfPCell(new Paragraph("会员卡充值金额合计", FontColumn));
            pcMemTotalCo2.HorizontalAlignment = Element.ALIGN_CENTER;
            pcMemTotalCo2.NoWrap = false;
            ptb.AddCell(pcMemTotalCo2);

            PdfPCell pcMemTotalCo3 = new PdfPCell(new Paragraph(NumericUtils.ToMoney(u.CashHandOver.MemberCardChargeAmount), FontColumnValue));
            pcMemTotalCo3.HorizontalAlignment = Element.ALIGN_RIGHT;
            pcMemTotalCo3.NoWrap = false;
            pcMemTotalCo3.Colspan = 3;
            ptb.AddCell(pcMemTotalCo3);

            //欠条合计
            PdfPCell pcPre4 = new PdfPCell(new Paragraph(Convert.ToString(index + 3), FontColumnValue));
            ...
            //备用金
            PdfPCell pcPre5 = new PdfPCell(new Paragraph(Convert.ToString(index + 4), FontColumnValue));
            ...
            str += " 会员卡充值金额合计:" + NumericUtils.ToMoney(u.CashHandOver.MemberCardChargeAmount);
            str += " 欠条:";
            str += " 备用金:";
