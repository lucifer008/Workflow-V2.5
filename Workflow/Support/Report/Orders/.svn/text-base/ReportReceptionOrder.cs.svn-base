...
namespace Workflow.Support.Report.Orders
{
    /// <summary>
    /// 名    称: ReportReceptionOrder
    /// 功能概要: 分配给接待人报表
    /// 作    者: 张晓林
    /// 创建时间: 2010年1月18日16:19:51
    /// 修正履历: 
    /// 修正时间: 
    /// 修正描述:
    /// </summary>
    public class ReportReceptionOrder : ReportBase<iTextSharp.text.pdf.PdfPTable, OrderModel>
    {
        ...
        private PdfPTable GetOrderItem(OrderModel u)
        {
            ...
            PdfPCell pc1 = new PdfPCell(new Paragraph("订单号:", FontHeader));
            pc1.HorizontalAlignment = 2;
            pc1.Border = 0;
            pc1.NoWrap = false;
            ptb.AddCell(pc1);

            PdfPCell pc2 = new PdfPCell(new Paragraph(u.NewOrder.No, FontHeader));
            pc2.Border = 0;
            pc2.NoWrap = false;
            pc2.HorizontalAlignment = Element.ALIGN_LEFT;
            ptb.AddCell(pc2);

            PdfPCell pc23 = new PdfPCell(new Paragraph("接待人:", FontHeader));
            pc23.HorizontalAlignment = 2;
            pc23.Border = 0;
            pc23.NoWrap = false;
            ptb.AddCell(pc23);

            PdfPCell pc24 = new PdfPCell(new Paragraph(u.NewOrder.ReceptionEmployee, FontHeader));
            pc24.Border = 0;
            pc24.NoWrap = false;
            pc24.HorizontalAlignment = Element.ALIGN_LEFT;
            ptb.AddCell(pc24);

            PdfPCell pc3 = new PdfPCell(new Paragraph("客户名称:", FontHeader));
            pc3.HorizontalAlignment = 2;
            pc3.Border = 0;
            ptb.AddCell(pc3);

            PdfPCell pc4 = new PdfPCell(new Paragraph(u.NewOrder.CustomerName, FontHeader));
            pc4.HorizontalAlignment = Element.ALIGN_LEFT;
            pc4.Border = 0;
            pc4.NoWrap = false;
            ptb.AddCell(pc4);

            PdfPCell pc5 = new PdfPCell(new Paragraph("条码号:", FontHeader));
            pc5.HorizontalAlignment = 2;
            pc5.Border = 0;
            pc5.NoWrap = false;
            ptb.AddCell(pc5);

            PdfPCell pc6 = new PdfPCell(new Paragraph(u.NewOrder.No, FontHeader));
            pc6.HorizontalAlignment = Element.ALIGN_LEFT;
            pc6.Border = 0;
            pc6.Colspan = 3;
            pc6.NoWrap = false;
            ptb.AddCell(pc6);

            PdfPCell pc7 = new PdfPCell(new Paragraph("", FontHeader));
            pc7.Colspan = 4;
            pc7.Border = 0;
            pc7.FixedHeight = 10;
            ptb.AddCell(pc7);
