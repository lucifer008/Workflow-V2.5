...
namespace Workflow.Service.LogCounterManage
{
    ...
    public class LogCountersServiceImpl : ILogCountersService
    {
        ...
        {
			throw new Exception("旧的方法已经不在使用");
            ...
            foreach (MachineWatchConversionPaper var in machineWatchConversionPaperList)
            {
                ...
                //制作数量=实际订单数+未完工数量+废单+补单-上次未完工数量
		...
		/// 创建时间: 2009.4.22
		/// </remarks>
		public IList<MachineType> GetNeedRecordWarchMachineAndMachineWatch()
		{
			return machineTypeDao.SearchNeedRecordMachineType();
		}
		...
		public RecordMachineWatch GetCanUsedRecordMachineWatch()
		{
			...
			{
				recordMachineWatch.InsertUser = ThreadLocalUtils.CurrentUserContext.CurrentUser.Id;
				recordMachineWatch.RecordDateTime = DateTime.Now;
		...
		{
			recordMachineWatchDao.UpdateCanUsedRecordMachineWatch();
		}

		#endregion

		#region 添加一项未完成订单用纸情况
		...
		/// 删除未完工订单用纸情况
		/// </summary>
		/// <param name="uncompleteOrderUsedPaperId">未完工订单用纸情况Id</param>
		/// <remarks>
		/// 作    者: 陈汝胤
		/// 创建时间: 2009.4.24
		/// </remarks>
		...
		{
			return machineWatchRecordLogDao.GetNeedVerifyRecordLog(recordMachineWatchId, machineTypeId);
		}
		...
		/// 创建时间: 2009.5.4
		/// </remarks>
		public int GetCompensateUsedPaperCount(long recordMachineWatchId,long machineTypeId, long paperId,long paperId2, string colorType, long paperId3)
		{
			int count = 0;

			IList<CompensateUsedPaper> compenstateUsedPaperList = compensateUsedPaperDao.SelectCompensateUsedPaper(recordMachineWatchId, machineTypeId, paperId, paperId2, paperId3, colorType);
		...
		public void DailyRecordMachine(IList<MachineWatchConversionPaper> iList,long recordMachineWatchId)
		{
			...
			foreach (MachineWatchConversionPaper val in iList)
			{
				...
				dailyRecordMachine.RecordMachineWatchId = recordMachineWatchId;
				dailyRecordMachine.MachineTypeId = val.MachineTypeId;
				dailyRecordMachine.PaperSpecificationId = val.PaperId;
		...
		public IList<Order> GetNeedRecordMachineWatch(RecordMachineWatch lastRecordWath)
		{
			return orderDao.SelectNeedRecordMachineWatch(lastRecordWath);
		}
		...
		public IList<CompensateUsedPaper> GetCompensateUsedPaperByRecordWatchId(long recordWatchId)
		{
			return compensateUsedPaperDao.SelectCompensateUsedPaper(recordWatchId);
		}
		...
		{
			IList<RecordMachineWatch> vals = recordMachineWatchDao.GetRecordMachineWatch(beginRow, endRow, beginDate, endDate);
			foreach (RecordMachineWatch val in vals)
			{
				val.InsertUserName = userDao.GetEmployeeNameByUserId(val.InsertUser);
			}
			return vals;
		}
		...
		{
			return recordMachineWatchDao.GetRecordMachineWatchCount(beginDate, endDate);
		}
		...
		{
			return dailyRecordMachineDao.GetRecordMachineWatchResult(recordMachineWatchId);
		}
		...
		#region 通过纸型名称获取纸型Id
		/// <summary>
		/// 名    称: GetPaperSpecificationIdByName
		/// 功能概要: 通过纸型名称获取纸型
		/// 作    者: 
		/// 创建时间: 
		/// 修 正 人: 白晓宇
		...
		/// <returns>纸型ID</returns>
		public long GetPaperSpecificationIdByName(string name)
		{
			return paperSpecificationDao.SelectPaperSpecificationIdByName(name);
		}
		#endregion
	}
}
