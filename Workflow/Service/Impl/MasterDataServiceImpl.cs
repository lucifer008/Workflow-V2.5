...
namespace Workflow.Service.Impl
{
    public class MasterDataServiceImpl : Workflow.Service.IMasterDataService
    {
        ...
        /// 授权记录
        /// </summary>
        private IAccreditRecordDao accreditRedcordDao;
        public IAccreditRecordDao AccreditRedcordDao 
        {
            set { accreditRedcordDao = value; }
            get { return accreditRedcordDao; }
        }
        #endregion
        ...
        {
            bool isSystemMaintenance = user.IsSystemMaintenance;
            user = userDao.CheckUserNameAndPassword(user);
            if (null == user) return 1;                                          //用户名或密码错误
            if (user.IsLogin == Constants.TRUE && !isSystemMaintenance) return 2;//前台用户已经登陆
            ...
            if (!isSystemMaintenance)
            {
                ...
                userDao.UpdateUserLoginStatus(user);//LogoutUser(user);
            }
            return 3;
        }
        #endregion
        ...
        public int LogoutLoginUser(User user)
        {
            ...
            if (u.IsLogin == Constants.TRUE)
            {
                ...
                userDao.UpdateUserLoginStatus(u);//LogoutUser(u);
        ...
        ///  修正时间:
        /// </remarks>
        public void InsertAccreditRecord(AccreditRecord accreditRecord) 
        {
            accreditRedcordDao.Insert(accreditRecord);
        }
        #endregion
        ...
        {
            long branchShopId = user.BranchShopId;
            user.PositionId = Workflow.Support.Constants.POSITION_MANAGER_VALUE(branchShopId);
            return userDao.CheckIsBestUser(user);
        }
        #endregion
    }
